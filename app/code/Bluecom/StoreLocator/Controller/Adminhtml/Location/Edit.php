...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Bluecom_StoreLocator::store_location')
            ->addBreadcrumb(__('Store Location'), __('Store Location'))
            ->addBreadcrumb(__('Manage Store Location'), __('Manage Store Location'));
        return $resultPage;
    }
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('store_location', $model);

        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Store Location') : __('New Store Location'),
            $id ? __('Edit Store Location') : __('New Store Location')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Store Locations'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Store Location'));
